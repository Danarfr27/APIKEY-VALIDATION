...
def main():
    ...
    # write active keys to file (aktif.txt) and invalid keys to (invalid.txt)
    try:
        keyfile_dir = Path(args.file).resolve().parent
        aktif_path = keyfile_dir / 'aktif.txt'
        invalid_path = keyfile_dir / 'invalid.txt'
        ...
        with invalid_path.open('w', encoding='utf-8') as inf:
            for r in results_sorted:
                if not r['ok']:
                    # if status is None include as invalid as well
                    inf.write(f"{r['key']},\n")
                    invalid_count += 1

        print(f'Active keys written to: {aktif_path} ({active_count})')
        print(f'Invalid keys written to: {invalid_path} ({invalid_count})')
    except Exception as e:
        print('Failed to write aktif/invalid files:', e, file=sys.stderr)
